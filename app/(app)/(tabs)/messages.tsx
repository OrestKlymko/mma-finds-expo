                    ...
                    onPress: async () => {
                        try {
                            await runTransaction(db,async tx => {
                                ...
                                const messagesSnap = await getDocs(collection(convRef, 'messages'));
                                messagesSnap.forEach(m => tx.delete(m.ref));
                                tx.delete(convRef);
                            });
                        } catch (error) {
            ...
            <Pressable
                ...
                <View style={styles.messageContent}>
                    <Text
                        style={[
                            styles.sender,
                            (data?.item?.unreadCount || 0) > 0 && styles.unreadSender,
                        ]}>
                        {data.item.sender}
                    </Text>
                    <Text
                        style={[
                            styles.messageText,
                            (data?.item?.unreadCount || 0) > 0 && styles.unreadMessageText,
                        ]}
                        numberOfLines={1}>
                ...
                <Text
                    style={[
                        styles.time,
                        (data?.item?.unreadCount || 0) > 0 && {fontWeight: 'bold'},
                    ]}>
