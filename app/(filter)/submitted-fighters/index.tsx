...
import {getCountriesForSubmittedFighter, getFoundationStylesFilterOfferById,} from '@/service/service';
...
const FilterScreen = () => {
    ...
    useEffect(() => {
        ...
        Promise.all([
            getFoundationStylesFilterOfferById(availableFilters.offerId),
            getCountriesForSubmittedFighter(availableFilters.offerId),
        ])
            .then(([foundationStyles, cit]) => {
                setFoundationStyles(foundationStyles);
                setCities(cit);
            })
            .finally(() => {
                setContentLoading(false);
            });
    }, [availableFilters.offerId]);
    ...
    function toggleFilter<K extends keyof SubmittedFilterFighter>(
        category: K,
        value: SubmittedFilterFighter[K] extends (infer U)[] ? U : never,
    ) {
    ...
    const clearAllFilters = () => {
        setAvailableFilters({
            locations: [],
            foundationStyle: [],
            offerId: availableFilters.offerId,
            withTapology: false,
        });
    };
                    ...
                    <View style={styles.filterContainer}>
                        {cities.slice(0, 5).map(place => (
                            <FilterButton
                                ...
                                selected={availableFilters?.locations?.includes(place.country)}
                                onPress={() => toggleFilter('locations', place.country)}
                            />
                        ))}
                        {cities.length > 5 && (
                            <TouchableOpacity
                                onPress={() => handleMorePress('locations')}
                    ...
                    <View style={styles.filterContainer}>
                        {foundationStyles.slice(0, 5).map(style => (
                            <FilterButton
                                key={style.id}
                                label={style.name}
                                selected={availableFilters?.foundationStyle?.includes(
                                    style.name,
                                )}
                    ...
                    <View style={styles.matchTypeContainer}>
                        <TouchableOpacity
                            style={[
                                ...
                                availableFilters?.withTapology === false &&
                        ...
                        <TouchableOpacity
                            style={[
                                styles.matchTypeButton,
                                availableFilters?.withTapology === true &&
                                styles.matchTypeButtonActive,
                            ]}
                            onPress={() =>
                                setAvailableFilters(prev => ({
